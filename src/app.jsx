...
class App extends Component {
  ...
  handleCalculation() {
    const due = (this.state.amountDue * 100);
    const received = (this.state.amountReceived * 100);
    const changeDue = ((received - due) / 100).toFixed(2);
    const subTotal = (received - due);
    const twenties = Math.floor((subTotal) / 2000);
    const tens = Math.floor((subTotal % 2000) / 1000);
    const fives = Math.floor((subTotal % 1000) / 500);
    const ones = Math.floor((subTotal % 500) / 100);
    const quarters = Math.floor((subTotal % 100) / 25);
    const dimes = Math.floor((subTotal % 25) / 10);
    const nickels = Math.floor((subTotal - twenties * 2000 - tens * 1000 - fives * 500 - ones * 100 - quarters * 25 - dimes * 10) / 5);
    const pennies = Math.ceil((subTotal % 5) / 1);
    this.setState({
